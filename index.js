...
function AssetsPlugin(DM, options) {
  if (!options.addBinaryAsset || !options.fbaTypes) {
    throw new Error(
      "addBinaryAsset function and array of FBA types are needed to load binary assets into FBA payload"
    );
  ...
  this.loadBinaryImports = createBinaryImporter(
    options.fbaTypes,
    options.addBinaryAsset
  );
...
AssetsPlugin.prototype.apply = function(compiler) {
  ...
  compiler.hooks.afterCompile.tapAsync(pluginName, (compilation, callback) => {
    ...
    const buildModule = compilation.entries.find(
      m => m.resource && m.resource === buildEntry
    );
    /* 
    ...
    const resourcesOnBuildEntry = getDepsFromModule(
      buildEntry,
      modulesByResource
    );
    ...
    /* 
			...
			as a ModuleReason
		*/
    const resourcesWithBuildEntryReason = getDepsWithModuleReason(
      compilation.modules,
      buildModule
    );
  ...
  compiler.hooks.emit.tapAsync(pluginName, (compilation, callback) => {
    ...
    for (var i in this.options.assets) {
      ...
      if (payload.type.match(/^fbA/i)) {
        ...
        promises.push(
          copier.copy(payload.modules, this.options.assets[i].copy)
        );
      }
    }

    // compile all the assets
    if (!this.options.base64Inline && fbaAssets.length) {
      promises.push(
        fbaCompiler.compile({
          target: path.resolve(
            `${this.options.output.path}/${this.options.output.filename}`
          ),
          assets: fbaAssets
        })
      );
