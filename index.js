...
AssetsPlugin.prototype.apply = function(compiler) {
  ...
  compiler.hooks.afterCompile.tapAsync(pluginName, (compilation, callback) => {
    ...
    /*
      Get modules which have the build entry as the issuer
      */
    const modulesWithIssuerBuildEntry = compilation.modules
      .filter(function checkIssuer(m) {
        if (!m.issuer) return false
        // log('m.issuer.resource', m.issuer.resource)
        return m.issuer.resource === buildEntry || checkIssuer(m.issuer)
      })
      .map(m => m.resource)

    const allDeps = resourcesOnBuildEntry
      .concat(resourcesWithBuildEntryReason)
      .concat(modulesWithIssuerBuildEntry)
